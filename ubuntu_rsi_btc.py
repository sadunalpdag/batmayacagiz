...
def price():
    ...
    bbandhigh.append(bbandhigh_mat)
    bbandmid.append(bbandmid_mat)
    bbandlow.append(bbandlow_mat)


    bband_low = pd.DataFrame(bbandlow,columns=['bbandlow'])
    bband_mid = pd.DataFrame(bbandmid,columns=['bbandmid'])
    bband_high = pd.DataFrame(bbandhigh,columns=['bbandmid'])
    ...
    # print(macd3 )
    data2 = pd.concat([data, macd1, macd2, macd3,data3,data4,bband_low,bband_high,bband_mid], axis=1)

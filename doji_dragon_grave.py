    ...
    def dfall(self, symbol, timeframe):
        ...
        try:
            if self.shortgiris == 1 or self.longgiris == 1 or order_approve == 0:  # alıs satıstan sonra 100 cycledan sonra tekrar işleme açma
                ...
                target = 'bearish_gravestone_doji'
                candles_df = candlestick.gravestone_doji(candles_df, target=target)
                target1 = 'bullish_dragonfly_doji'
                candles_df = candlestick.dragonfly_doji(candles_df, target=target1)
                self.last_elementbullish = candles_df["bullish_dragonfly_doji"].iloc[-1]
                self.last_elementbearish = candles_df["bearish_gravestone_doji"].iloc[-1]
                ...
                if self.shortgiris != 1 and self.longgiris != 1:
                    ...
                    document = db.collection("doji" + timeframe).document(self.symbol)
                    ...
                    document.set({
                        "symbol": self.symbol,
                        "doji_dragon": last_elementbullish_fire,
                        "doji_grave": last_elementbearish_fire,
                            ...
                            if order_approve == 1:
                                ...
                                if balance > 200:
                                    try:
                                        if self.last_elementbullish == True:
                                            print('long gir')
                                            tele.telegram_bot('long gir-doji_dragon_strategy')
                                            ...
                                            tele.telegram_bot("short_gir-goji_grave_strategy")
                                            tele.telegram_bot(symbol)
                                            self.shortgiris += 1
